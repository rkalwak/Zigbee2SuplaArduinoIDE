...
void ZigbeeGateway::zbAttributeRead(esp_zb_zcl_addr_t src_address, uint16_t src_endpoint, uint16_t cluster_id, const esp_zb_zcl_attribute_t *attribute)
{
  ...
  if (attribute->data.type == ESP_ZB_ZCL_ATTR_TYPE_S16) // signed 16bit
  {
    esp_zb_zcl_attr_s* attr = esp_zb_zcl_get_attribute(src_endpoint, cluster_id, ESP_ZB_ZCL_CLUSTER_CLIENT_ROLE, attribute->id); // other role is ESP_ZB_ZCL_CLUSTER_SERVER_ROLE
    log_i("Value of parameter %d", attr->data_p);
  }
  else if (attribute->data.type == ESP_ZB_ZCL_ATTR_TYPE_U16) // unsigned 16 bit
  {
  }
}
